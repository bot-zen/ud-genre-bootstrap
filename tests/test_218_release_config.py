...
def test_218_community_release_config_has_promoted_identity():
    ...
    assert identity["artifact_id"] == "ud2.18-full-ud-v1.0.1"
    assert identity["hf_branches"] == ["2.18"]
    assert identity["hf_tag"] == "artifact/ud2.18-full-ud-v1.0.1"
    assert identity["source_branch"] == "release/v1"
    assert identity["source_tag"] == "source/ud2.18-full-ud-v1.0.1"
