...
MATRIX = "configs/releases/full-ud-v1.0.0.yaml"
...
def test_release_matrix_resolves_default_ud_version():
    ...
    assert cfg.output.run_id == "full-ud-v1.0.0-ud2.18"
    ...
    assert identity["hf_revisions"] == ["2.18", "artifact/full-ud-v1.0.0/ud2.18"]
    assert identity["source_tag"] == "source/full-ud-v1.0.0"
    assert getattr(cfg, "_release_matrix_path").endswith("full-ud-v1.0.0.yaml")
